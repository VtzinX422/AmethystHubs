...
-- ==========================================
-- 1. INTERFACE DE COORDENADAS FIXA (BEM NO TOPO)
-- ==========================================
local realTimeFrame = Instance.new("Frame", screenGui)
realTimeFrame.Size = UDim2.new(0, 180, 0, 45)
realTimeFrame.Position = UDim2.new(0.5, -90, 0, -15) 
...
rtLabel.Size = UDim2.new(1, 0, 1, 0); rtLabel.BackgroundTransparency = 1; rtLabel.TextColor3 = Color3.new(1, 1, 1); rtLabel.Font = Enum.Font.Code; rtLabel.TextSize = 12; rtLabel.RichText = true
rtLabel.Text = "X: 0 | Y: 0 | Z: 0"

local btnCopyRT = Instance.new("TextButton", realTimeFrame)
btnCopyRT.Size = UDim2.new(0, 50, 0, 20); btnCopyRT.Position = UDim2.new(1, -55, 1, -20); btnCopyRT.BackgroundTransparency = 1; btnCopyRT.Text = "copiar"; btnCopyRT.TextColor3 = Color3.fromRGB(0, 120, 255); btnCopyRT.TextSize = 11; btnCopyRT.Font = Enum.Font.SourceSansBold
local textStroke = Instance.new("UIStroke", btnCopyRT); textStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; textStroke.Color = Color3.new(1, 1, 1); textStroke.Thickness = 1.5

btnCopyRT.MouseButton1Click:Connect(function()
    local pos = getFeetPos()
    local text = string.format("X: %.1f\nY: %.1f\nZ: %.1f", pos.X, pos.Y, pos.Z)
...
mainFrame.Position = UDim2.new(0.5, -190, 0, 50) 
...
titleTxt.Text = "  VoidAmethyst Hub"; titleTxt.Size = UDim2.new(0.4, 0, 1, 0)
titleTxt.TextColor3 = Color3.new(1,1,1); titleTxt.BackgroundTransparency = 1; titleTxt.Font = Enum.Font.SourceSansBold; titleTxt.TextXAlignment = Enum.TextXAlignment.Left; titleTxt.ZIndex = 6; titleTxt.TextSize = 16

local devTxt = Instance.new("TextLabel", titleBar)
devTxt.Text = "⚙️ Dev Vtzin x | ©VTG solutions"; devTxt.Size = UDim2.new(0.4, 0, 1, 0)
devTxt.Position = UDim2.new(0.35, 0, 0, 0) 
...
btnMarcar.MouseButton1Click:Connect(function()
    ...
    conexaoUpdate = RunService.RenderStepped:Connect(function()
        ...
        if not cuboPreview then 
            ...
            local out = Instance.new("SelectionBox", cuboPreview)
            out.Adornee = cuboPreview
            out.Color3 = corAcento -- ROXO AMETISTA
            out.LineThickness = 0.15
        end
...
btnFechar.MouseButton1Click:Connect(function() if conexaoUpdate then conexaoUpdate:Disconnect() end; lblF.Text = "<b>FIM</b>\n"..format(getFeetPos()) end)
btnLimpar.MouseButton1Click:Connect(function() if cuboPreview then cuboPreview:Destroy(); cuboPreview = nil end; pontoA = nil end)
local btnExt = Instance.new("TextButton", screenGui); btnExt.Size = UDim2.new(0, 50, 0, 50); btnExt.Position = UDim2.new(1, -60, 0.05, 0); btnExt.BackgroundColor3 = Color3.fromRGB(15,15,20); btnExt.Text = "💎"; btnExt.ZIndex = 100000; Instance.new("UICorner", btnExt).CornerRadius = UDim.new(1, 0); btnExt.MouseButton1Click:Connect(function() mainFrame.Visible = not mainFrame.Visible end)
